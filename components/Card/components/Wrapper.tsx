...
const Wrapper = ({
  ...
  return (
    <Link href={`/${type ? 'movie' : 'tv'}/[id]`} as={`/${type ? 'movie' : 'tv'}/${id_db}`}>
      {isLoading ? (
        <div className='w-full grid place-items-center relative loading-animation rounded-8 overflow-hidden my-1'>
          <div style={{ aspectRatio: '2 / 3', width: '100%' }} className='no-drag select-none w-full overflow-hidden relative'></div>
        </div>
      ) : (
        <a
          className={`flex flex-col flex-1 cursor-pointer relative overlay rounded-10 my-1 ${
