...
const Rating = ({ vote_average, notchild, className, ...props }: RatingProps) => {
  const nr = !vote_average;

  return (
    <div
      className={`${
        !notchild ? 'absolute top-1 left-1 z-20' : 'w-10'
      } p-1 flex items-center justify-center bg-black bg-opacity-80 rounded-5 ${nr ? 'opacity-60' : ''} ${className}`}
      {...props}
    >
