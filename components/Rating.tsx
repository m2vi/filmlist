...
const Rating = ({ vote_average, state, notchild, className, ...props }: RatingProps) => {
  const nr = !vote_average || typeof vote_average !== 'number';
  return (
    <div className={`${!notchild ? 'absolute top-2 right-2 z-20' : 'w-10'}`} {...props}>
      <div className='flex justify-center items-center h-36 w-36 relative'>
        <span className='text-xs font-medium text-button l-1'>{nr ? 'NR' : vote_average.toFixed(1)}</span>
        <svg x='0px' y='0px' width='36px' height='36px' viewBox='0 0 36 36' className='absolute -inset-2px h-8 w-8'>
          <circle
            ...
            cy='18'
            r='16'
            strokeDasharray={`${nr ? '100' : vote_average * 10} 100`}
            strokeDashoffset='0'
            strokeLinecap='round'
